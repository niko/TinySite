...
Each page is rendered into a "HAML"http://haml-lang.com layout file. The parts of the page and a set of variables are provided by the page itself and a global content file. The layout file should be located next to the rackup file @config.ru@ and defaults to @layout.haml@. The layout file is configurable via @:layout@ @page_vars@ and @global_vars@.
...
Analog the the page_vars and page_parts the global_vars and global_parts are set via the content of the @__global.textile@ file. A typical one would perhaps look like this:
...
The file path would be the folder in which the page content files are available. Other supported parameters are @:image_path@ (defaults to @[file_path]/images@) and @cache_buster@ (a query string with which you can bust the internal cache and prevent HTTP caching for the given site).
